...
if [[ "$OSTYPE" == "msys" ]] || [[ "$MSYSTEM" != "" ]] || [[ "$OS" == "Windows_NT" ]] || [[ "$RUNNER_OS" == "Windows" ]] || [[ "$CI" == "true" && $(uname -s 2>/dev/null || echo '') == *"MSYS"* ]]; then
    ...
    # Build libatari800 library (minimal version - disable XEP80 to avoid build issues)
    ./configure --target=libatari800 --enable-netsio --disable-xep80
fi

echo "atari800 configuration completed"

# Skip XEP80 forcing for now to avoid build issues
